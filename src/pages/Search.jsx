...
const Search = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8080/products?q=${encodeURIComponent(e.target.search.value)}&_page=${currentPage}`
      );
      ...
      setProducts([]); // Clear products or handle error state
    }
  };
  ...
  const handleSearchPagination = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/products?q=${encodeURIComponent(searchTerm)}&_page=${currentPage}`
      );
      const data = response.data;
      if (Array.isArray(data)) {
        setProducts(data);
